"""
train_sequential_fast.py — Stratified 5-fold CV training of SequentialHead on
cached EfficientNet-B4 + ViT-B/16 features.

Why k-fold CV?
    SDFVD is small (~106 videos). Any single random train/val split has high
    variance — accuracy can swing 20+ points based on which videos land in val.
    Reporting a single number from one split is not defensible. Standard
    practice for this dataset class is k-fold CV mean ± std, which is also
    what reviewers expect for small-corpus deepfake-detection papers.

What it does:
    1. Loads the train and val feature caches and concatenates them.
    2. Stratified 5-fold split (real:fake balanced in every fold).
    3. Trains a fresh SequentialHead per fold from random init.
    4. Reports per-fold val accuracy + F1 + mean ± std.
    5. Saves the best fold's model for inference.
...
from models.cached_heads import SequentialHead
...
def smoothed_bce_loss(preds: dict, targets: torch.Tensor,
    ...
    """BCE-with-logits + label smoothing + auxiliary GAN/ViT branch losses."""
    if targets.dim() == 1:
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  SEQUENTIAL HEAD — {args.folds}-Fold Stratified CV")
    print(f"{'='*70}")
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(n_total), labels_np), start=1):
        ...
        if fold_acc > best_global_acc:
            ...
            best_payload = {
                ...
                'all_fold_accs': None,           # filled below
                'all_fold_f1s': None,
                'cv_mean_acc': None,
                'cv_std_acc': None,
                'args': vars(args),
    ...
    if best_payload is not None:
        ...
        torch.save(best_payload, os.path.join(SAVE_DIR, 'SequentialHead_best.pth'))

    print(f"\n{'='*70}")
    print(f"  CROSS-VALIDATION RESULTS (SequentialHead)")
    print(f"{'='*70}")
    ...
    print(f"  Checkpoint : {SAVE_DIR}/SequentialHead_best.pth")
    print(f"{'='*70}\n")
